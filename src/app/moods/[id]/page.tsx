...
import { updateMoodTagDetailsAction } from "@/server/actions/rules";
import { summarizeTrades } from "@/server/queries/analytics";
import { getMoodTagById } from "@/server/queries/rules";
import { listTrades } from "@/server/queries/trades";

const pct = (v: number | null) => (v == null ? "—" : `${Math.round(v * 100)}%`);
const rMultiple = (v: number | null) => (v == null ? "—" : `${v.toFixed(2)}R`);

export default async function MoodPlaybookPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const [tag, trades] = await Promise.all([getMoodTagById(id), listTrades({ moodTagId: id })]);
  if (!tag) notFound();
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Mood playbook</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <form action={updateMoodTagDetailsAction.bind(null, tag.id)} className="flex flex-col gap-4">
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="What this mood tends to lead to, and what to do about it..."
              />
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Trades entered in this mood</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {trades.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-muted-foreground">
                      No trades entered in this mood yet.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
